...
{
    public partial class FrHistoryDataCollect : Form
    {

        private readonly string XAP_URL = ConfigurationManager.AppSettings["dataUrl"];

        public static FrHistoryDataCollect frDataCollect;
        ...
        {
            startTime.MaxDate = DateTime.Now;
            endTime.MaxDate = DateTime.Now;
        }
        ...
        private void setControlStatus(bool status)
        {
            try
            {
                ...
                this.startTime.Enabled = status;
                this.endTime.Enabled = status;
            }
            catch (Exception ex)
            {
                //日志处理
        ...
        /// </summary>
        /// <returns></returns>
        public async Task collectHourDataTool(List<DateTime> collectTimeList)
        {
            ...
            try
            {
                ...
                for (int t = 0; t < collectTimeList.Count; t++) 
                {
                    DateTime cTime = collectTimeList[t];
                                        ...
                                        if (cityAQILiveDataIEB != null)
                                        {
                                            ...
                                            if (cityAQILiveDataList != null)
                                            {
                                                for (int j = 0; j < cityAQILiveDataList.Count; j++)
                                                {
                                                    ...
                                                    {
                                                        bool isCompeletCollect = cmhr.IsCompeletCollect(hourCity, cityCode, cTime);
                                    ...
                                    if (stationAQILiveData != null)
                                    {
                                        ...
                                        if (stationAQILiveData != null && stationAQILiveData.Length > 0)
                                        {
                                            for (int j = 0; j < stationAQILiveData.Count(); j++)
                                            {
                                                ...
                                                {
                                                    bool isCompeletCollect = smhr.IsCompeletCollect(hourStation, stationCode, cTime);
                                                    if (!isCompeletCollect)
                                                    {
                                                        JObject item = new JObject();
        ...
        public List<DateTime> collectTimeList() 
        {
            ...
            try
            {
                ...
                {
                    sTime = DateTime.Parse(startTime.Value.ToString("yyyy-MM-dd 00:00:00"));
                    eTime = DateTime.Parse(endTime.Value.ToString("yyyy-MM-dd 23:00:00"));
                    for (DateTime dt = sTime; dt <= eTime; dt = dt.AddHours(1))
                    {
                        this.rtb_Log.AppendText(dt.ToString("yyyy-MM-dd HH:mm:ss")+ "\r\n");
                        list.Add(dt);
                    }
                }
                else if (dayRadio.Checked)
                {
                    sTime = DateTime.Parse(startTime.Value.ToString("yyyy-MM-dd 00:00:00"));
                    eTime = DateTime.Parse(endTime.Value.ToString("yyyy-MM-dd 00:00:00"));
                    for (DateTime dt = sTime; dt <= eTime; dt = dt.AddDays(1))
                    {
                        this.rtb_Log.AppendText(dt.ToString("yyyy-MM-dd HH:mm:ss") + "\r\n");
                        list.Add(dt);
                    }
